...
if [[ ! "$SKIP_STEP_1" = "true" ]]; then
  ...
  if [[ ! `docker ps | grep $OP8_CONTAINER` ]]; then
    docker run -d --name $OP8_CONTAINER openproject/community:8.3.0 # can use `run -it` directly because the image doesn't support it yet in version 8
    if [[ $? -gt 0 ]]; then exit 1; fi
    echo "  OpenProject started"
  else
    echo "  OpenProject already running"
  ...
  #
  # Note that the below list is JUST AN EXAMPLE, you should see what
  # migrations the 1.7 step complains about (it will list them out) and add
